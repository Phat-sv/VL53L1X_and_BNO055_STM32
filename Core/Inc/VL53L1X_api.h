...
#define VL53L1_ERROR_TIMEOUT (10)
#define VL53L1_BOOT_TIMEOUT  (20)
...
/**
 * @brief This function sets the sensor I2C address used in case multiple devices application, default address
 * 0x52
 */
 ...
 * The ranging operation is continuous. The clear interrupt has to be done after each get data to allow the
 * interrupt to raise when the next data is ready\n 1=active high (default), 0=active low, use
 * SetInterruptPolarity() to change the interrupt polarity if required.
 */
...
/**
 *@brief This function programs the new user ROI center, please to be aware that there is no check in this
 * function. if the ROI center vs ROI size is out of border the ranging function return error #13
 */
...
//===============================================//
/*
 * @brief set the VL53L1 Xshutdown pin, waiting for a complete boot sequence (if timeout, return error)
 * @return
 *  - 0: successful
 *  - 1: failed
 */
