...
const uint8_t VL51L1X_DEFAULT_CONFIGURATION[] = {
  ...
  0x20, /* 0x46 : interrupt configuration 0->level low detection, 1-> level high, 2-> Out of window, 3->In
  ...
  0x01, /* 0x64 : Sigma threshold MSB (mm in 14.2 format for MSB+LSB), use SetSigmaThreshold(), default value
           90 mm  */
...
VL53L1X_ERROR VL53L1X_SensorInit(uint16_t dev)
{
  ...
  if (timeout == 0)
  {
    return VL53L1_ERROR_TIMEOUT;  // timeout = 1000ms
  }
...
{
  uint8_t sensorStatus = 0;  //(1:booted, 0:not booted)
  uint8_t status       = 0;
  HAL_GPIO_WritePin(TOF_XSHUT_GPIO_Port, TOF_XSHUT_Pin, (GPIO_PinState)level);
  ...
  if (level == 1)
  {
    ...
    {
      HAL_Delay(2);
      status |= VL53L1X_BootState(VL53L1__ADDR, &sensorStatus);  // The boot duration is 1.2 ms maximum

      if (HAL_GetTick() - start > 100)
      {
        return VL53L1_BOOT_TIMEOUT;  // if timeout (100ms) --> return 20
      }
    }
  }
  return status;
...
// Enable VL53L1 sensor, waiting for a complete boot sequence, if timeout --> return 20
  ...
  status |= VL53L1X_SetDistanceThreshold(VL53L1__ADDR,
                                         VL53L1__LOWER_THRESHOLD,
                                         VL53L1__UPPER_THRESHOLD,
                                         VL53L1__WINDOW_MODE,
                                         0);

  /* set interrupt polarity to low */
  status |= VL53L1X_SetInterruptPolarity(VL53L1__ADDR, IT_POLARITY_LOW);

  /* set ROI size and ROI center */
  status |= VL53L1X_SetROI(VL53L1__ADDR, ROI_X_SIZE, ROI_Y_SIZE);
  // status |= VL53L1X_SetROICenter(VL53L1__ADDR, ROI_CENTER);
